...
obs3$ecoregion[obs3$ecoregion=="arcticocean"]<-"arctic ocean"
obs3$ecoregion[obs3$ecoregion=="bay of biscay and iberian coast"]<-"bay of biscay and the iberian coast"
...
fwrite(bpue1, file = "data/bpue1_uncensoredlist.csv", sep = ";",na="NA")

###first issue with ecoregion BoB & AO name in new list....

bpue1 <- bpue1[!ecoregion %like% "arcticocean"]
bpue1 <- bpue1[!ecoregion %like% "bay of biscay and iberian coast"]
...
################################################################################
################################################################################
##### 2026 Common Dolphin Case study #####
...
#Total per year and per quarter CD 
...
# Reliability checks for CD
```{r}
# Using WKMAMBY script for reliability checks, it is a improved version
...
This step was introduced during the benchmark in Dec 2025 (see WKBBEAM report). Here we attempt to estimate whether we are comfortable with instances when we have no bycatch events monitored. That is, we aim to estimate whether the probability that a bycatch would have occurred but was not monitored is low. We tested varying approaches during the benchmark to achieve this. Here we assume that we would like to be sure that we are able to detect very rare bycatch (BPUE = 0.001) and that we do that with virtual certainty (probability of 0.01 that we observe no bycatch given the monitoring effort spent and the true BPUE is 0.001).
If a case meets this requirement then BPUE is estimated to by zero with a lower confidence interval of zero and an upper confidence interval estimated using a parametric approach (adjusted Wald method) and the bycatch estimate is zero.
...
```{r, eval=FALSE, echo=FALSE, out.width="90%"}
