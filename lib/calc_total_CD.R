...
calc_total_CD <- function(bpue, analysis_resolution, dat, fishing, verbose = TRUE, include.weights=FALSE,response, effort_term,weights_values=NULL, filter=NULL) {
  ...
  dat <- dat[bpue, on = analysis_resolution]
  dat <- dat[effort > 0]
  ...
  #dat[, weights := ifelse(year >= (max(years)-4),1,0.5)] OLD VERSION, KEEP AS AN EXAMPLE
    ...
    # don't try to generate total estimates when some random effects in the best model are not present in the total fishing effort.
    if (re.n > 0) {
      missing_re <- re[!re %in% names(fishing)]
      if (length(missing_re) > 0) {
        ret$message <- paste("random effect(s) not available in fishing effort data:", 
    ...
    fishing_filtered[, effort := get(effort_term)]
    tot <- fishing_filtered[bpue, on = analysis_resolution[analysis_resolution != "species"], .(effort = sum(effort)), by = re]
    tot <- tot[complete.cases(tot)]
    ...
    if (any(sapply(re, function(re) !all(tot[[re]] %in% unique(dat[[re]]))))) {
        ret$message <- "More levels available in fishing effort than in monitoring data for at least one random effect - Cannot estimate total bycatch for the levels in which BPUE is unknown"
        return(ret)
    }
    ...
    if (re.n == 0) {
        ...
        ret[, c("tot_mean", "tot_lwr", "tot_upr") := as.list(pred[, c("response", "asymp.LCL", "asymp.UCL")] + tot$observed_bycatch)]  # prediction for unmonitored fishing effort + observed bycatch in monitoring
        ...
        pred <- lapply(1:nrow(tot), function(i) {
          ...
          p <- ggpredict(model = best,
                           terms = tot[i, ..re],
                           condition = c(logEffort = tot$logEffort[i]),
                           type = type_arg,
                           interval = "confidence",
                           verbose = verbose)
        ...
        ret[, c("tot_mean", "tot_lwr", "tot_upr") := as.list(colSums(pred) + sum(tot$observed_bycatch))] # prediction for unmonitored fishing effort + observed bycatch in monitoring
        ret$fishing_effort <- sum(tot$effort) # retain total fishing effort or total unmonitored fishing effort?
